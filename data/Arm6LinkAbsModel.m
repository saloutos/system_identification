...
rotor_offsets = [0, 0, -0.02125;
                ...
                0,0,0]; %0, 0.01, -0.205]; % relative to elbow frame

rotor_offsets(1:4,:) = rotor_offsets(1:4,:) + joint_offsets(1:4,:);
rotor_offsets(5,:) = rotor_offsets(5,:) + joint_offsets(4,:);
rotor_offsets(6,:) = rotor_offsets(6,:) + joint_offsets(6,:); % TODO: change to joint_offsets(,:) eventually
...
model.joint{Nb} = revolutePairAbsoluteWithRotors();
...
model.joint{Nb}.XtreeInternal = plux(eye(3), joint_offsets(5,:)'); % second joint offset
...
model.I{Nb}(13:18,13:18) = mcI(rotor_masses(2),[0 0 0],boxInertia(rotor_masses(2),[U10_dia, U10_dia, U10_width])); % second rotor inertia 
model.I{Nb}(19:24,19:24) = mcI(link_masses(5),link_centers(5,:),boxInertia(link_masses(5),wrist_box)); % second link inertia
